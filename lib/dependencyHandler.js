...
const { readCache, writeCache } = require('./config.js');
...
async function installDependencies(language, scriptDir) {    
    ...
    const currentHash = await computeHashFile(depFile); 
    const cache = await readCache(scriptDir);

    if (cache && cache.sha256 == currentHash) {
        console.log('Dependencies unchanged (cache hit). Skipping install.');
        return true;
    }
    ...
    install_process.on("close", async (code) => {
        if (code === 0) { 
            console.log('Dependencies installed successfully. \n'); 
            await writeCache(scriptDir, currentHash); 
            return resolve(true); 
        } else { 
